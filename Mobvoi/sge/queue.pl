...
for my $option (keys %cli_options) {
  #print STDERR "in for option\n";
  if ($option eq "config") { next; }
  if ($option eq "max_jobs_run" ) {
    if ($array_job != 1) { next; }
    $steps = $cli_options{$option};
  }
  #print STDERR "steps=";
  #print $steps;
  #print "\n";
...
sub submit_jobs {
    ...
    $jobend = $joball if $joball < $jobend;
    ...
    ($queue_scriptfile =~ s/\.[a-zA-Z]{1,5}$/.$count.sh/) || ($queue_scriptfile .= ".sh");
    ...
    for (my $try = 1; $try < 5; $try++) {
        ...
        if ($ret != 0) {
            if ($sync && $ret == 256) {
                # this is the exit status when a job failed (bad exit status)
                if (defined $jobname) {
    ...
    if (! $sync) {
     ...
     $num_to_process = $num_to_process - ($jobend - $jobstart + 1);
     if($count > 1) {
         unlink($queue_scriptfile);
     }
...
# first commit $steps jobs and then listen to (@syncfiles is not empty)
submit_jobs(1, $steps);
while (scalar(@syncfiles) != 0) {
    for(my $index=0; $index < scalar(@syncfiles); $index++){
        my $f = $syncfiles[$index];
        if (-f $f) {
            splice(@syncfiles, $index, 1);
            unlink($f);
            if ($num_to_process > 0) {
                my $tmp_start = $joball - $num_to_process + 1;
                submit_jobs($tmp_start, $tmp_start);
            }
        }
    }
}
    ...
    if (!defined $jobname) { # not an array job.
      ...
      for (my $jobid = $jobstart; $jobid <= $joball; $jobid++) {
        my $l = $logfile;
    ...
    foreach my $l (@logfiles) {
      ...
      if ($status != 0) { $total_fail_num++; }
    }
    #$total_fail_num = $total_fail_num + $num_failed;
    if ($total_fail_num != 0) { # we failed.
      if (@logfiles == 1) {
        ...
        my $numjobs = 1 + $joball - $jobstart;
        print STDERR "queue.pl: $total_fail_num / $numjobs failed, log is in $logfile\n";
      }
      #exit(1);
    }
